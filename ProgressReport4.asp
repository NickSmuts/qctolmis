...
Set objRS = objDB.Execute("select n.NQName +'_' + d.project as NQName from NatQua as n left join DATA as d on d.NATQUA=n.NQName where not n.NQName='none' and not d.project='' group by n.NQName,d.project ")
...
' Create new CSV file 
csvFilePath ="C:\programfiles\ProgressReport.csv"
...
objCSVFile.Writeline
              'here we write headings to file

                objCSVFile.Write "Koue Bokkeveld Opleidingsentrum BK" & ","
               objCSVFile.Writeline
                objCSVFile.Write  "P.O.Box 56" & ","
               objCSVFile.Writeline
                objCSVFile.Write  "Koue Bokkeveld" & ","
               objCSVFile.Writeline
                objCSVFile.Write  "'6836'" & ","
               objCSVFile.Writeline
                objCSVFile.Write "Phone : 0233170983 /023 3170588" & ","
               objCSVFile.Writeline
               objCSVFile.Write  "Fax : 023 3170597/0867160325" & ","
               objCSVFile.Writeline
                objCSVFile.Write  "E-pos : jacob@kbos.co.za" & ","
               objCSVFile.Writeline
               objCSVFile.Writeline
               objCSVFile.Write "PROGRESS REPOR REF No."+Request.Form("qualification") & ","
               objCSVFile.Writeline
               objCSVFile.Writeline
               objCSVFile.Write "Trainer Name" & "," &  "Trainer Contact Number" & "," & "Company"
               objCSVFile.Writeline

              ' here we start writing first sheet Tutor Details
              dbname="data/learner.mdb"
        cnpath="DBQ=" & server.mappath(dbname)
        sDBName = "driver={Microsoft Access Driver (*.mdb)}; " & cnpath
        Set objDB = Server.CreateObject("ADODB.Connection")
        ...
        Set objRS = objDB.Execute("SELECT t.Tname,T.TSname FROM (DATA inner join learnerdata as l on data.Student_NUM=l.Student_NUM ) inner join teacher as t on t.tnoid=l.AssessorID WHERE data.NATQUA='"+split(Request.Form("qualification"),"_")(0)+"' and Data.project='"+split(Request.Form("qualification"),  "_")(1)+"' group by t.Tname,T.TSname")

        If objRS.EOF Then
        objCSVFile.Write "No Trainers found for Qualification"
        objCSVFile.Writeline
        objRS.Close
        objDB.Close
        Set objRS = Nothing
        ...
        Do While Not objRS.EOF
        'Html = html &"value=" & Chr(34) & objRS("NQName") & Chr(34) & ">" &objRS("NQName")
        objCSVFile.Write objRS("Tname") & " " & objRS("TSname") & "," & "-" & "," & "-"
        objCSVFile.Writeline
        objRS.MoveNext
        value = value +1
        Loop
        ...
        End If
              objCSVFile.Writeline
              objCSVFile.Writeline
        ' here learner data starts
                objCSVFile.Write ",,,Unitstandard numbers"
              objCSVFile.Writeline
               objCSVFile.Write "No" & "," & "Surname" & "," & "Name" & "," & "Electives" & "," & "Fundamentals" & "," & "Core" & "," & "N/A"
              objCSVFile.Writeline
            ...
            If objRS2.EOF Then
                  objCSVFile.Writeline
                  objCSVFile.Write "No Learners Found" & "," & "-"& "," & "-"& "," & "-"& "," & "-"& "," & "-"
                  objCSVFile.Writeline
                  objRS2.Close
            Set objRS2 = Nothing
            else
            ...
            Do While Not objRS2.EOF
                if sNumberOld = objRS2("Sname") & ";" & objRS2("Fname") OR sNumberOld = ""  then
                    ...
                    objCSVFile.Write rowCounter - 2 & "," & split(sNumberOld,";")(0) & "," & split(sNumberOld,";")(1) & "," & StudentE & "," & StudentF & "," & StudentC & "," & StudentNA
                    objCSVFile.Writeline
            ...
            objCSVFile.Writeline
            End If
              Response.Write ("File has been saved to C:\programfiles\ProgressReport.csv")
	            Case "old"
...
If objRS.EOF Then
	...
	'Response.End
            ...
            End If
              ...
              'Quit Excel
               objWorkbook.Saveas "c:\programfiles\testXLS" &Second(Now()) &".xlsx"
                objWorkbook.Close
