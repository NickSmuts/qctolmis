...
Set objRS = objDB.Execute("SELECT DATA.Sname, DATA.FName, DATA.ID_NUM, Project.EnrolldateY, Project.EnrolldateM, Project.EnrolldateD, DATA.Student_NUM, DATA.Race FROM (DATA INNER JOIN LearnerData ON DATA.Student_NUM = LearnerData.Student_NUM) INNER JOIN Project ON DATA.Project = Project.ProjectName WHERE (((LearnerData.Student_NUM)='"& request.form("StudentNum") & "'))")

Set objRS1 = objDB.Execute("SELECT LearnerData.EnrolldateY, LearnerData.EnrolldateM, LearnerData.EnrolldateD, LearnerData.STitle, Standards.SNumber, LearnerData.Student_NUM FROM LearnerData INNER JOIN Standards ON LearnerData.STitle = Standards.STitle WHERE (((LearnerData.Student_NUM)='"& request.form("StudentNum") & "'))")
...
Response.Write("<form method=POST action=QForm_Standards.asp>")
